...
class BufferPool
{
  public:
	...
	void Initialize (VkDevice device, const VkPhysicalDeviceMemoryProperties &mem_props, VkDeviceSize chunk_size = DEFAULT_CHUNK_SIZE);
	void Shutdown ();
...
class ImageMemoryPool
{
  public:
	...
	void Initialize (
		VkDevice device, const VkPhysicalDeviceMemoryProperties &mem_props, VkDeviceSize buffer_image_granularity, VkDeviceSize page_size = DEFAULT_PAGE_SIZE);
	void Shutdown ();

	bool Allocate (const VkMemoryRequirements &mem_reqs, VkMemoryPropertyFlags properties, ImageMemoryAllocation &out);
