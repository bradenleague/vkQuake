...
class CvarBindingManager
{
  public:
	...
	static void RegisterFloat (const char *cvar, const char *ui_name, float min = 0.0f, float max = 1.0f, float step = 0.1f);

	// Register a boolean cvar (0/1)
	static void RegisterBool (const char *cvar, const char *ui_name);

	// Register an integer cvar
	static void RegisterInt (const char *cvar, const char *ui_name, int min = 0, int max = 100);

	// Register an enum cvar (integer with fixed number of values)
	// labels is optional array of display names for each value
	static void RegisterEnum (const char *cvar, const char *ui_name, int num_values, const char **labels = nullptr);

	// Register an enum cvar with explicit value list (used for non-contiguous enums)
	static void RegisterEnumValues (const char *cvar, const char *ui_name, const std::vector<int> &values, const char **labels = nullptr);
