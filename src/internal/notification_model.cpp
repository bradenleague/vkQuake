...
void NotificationModel::RegisterBindings (Rml::DataModelConstructor &constructor)
{
	...
	constructor.BindFunc (
		"centerprint_visible", [] (Rml::Variant &variant)
		{ variant = !s_state.centerprint.empty () && (g_game_state.intermission || realtime < (s_state.centerprint_expire + CENTERPRINT_FLICKER_SECONDS)); });
	...
	constructor.BindFunc (
		...
		[] (Rml::Variant &variant)
		{
			variant = !s_state.centerprint.empty () && !g_game_state.intermission && realtime >= s_state.centerprint_expire &&
	...
	for (int i = 0; i < NUM_NOTIFY_LINES; i++)
	{
		...
		constructor.BindFunc (
			...
			[slot] (Rml::Variant &variant)
			{
				...
				variant = !line.text.empty () && line.time > 0.0 && elapsed < (notifytime + NOTIFY_FADE_SECONDS);
			});
	}
	...
	for (int i = 0; i < NUM_NOTIFY_LINES; i++)
	{
		...
		constructor.BindFunc (
			...
			[slot] (Rml::Variant &variant)
			{
				...
				const auto &line = s_state.notify[slot];
				double		elapsed = realtime - line.time;
				variant = !line.text.empty () && line.time > 0.0 && elapsed >= notifytime && elapsed < (notifytime + NOTIFY_FADE_SECONDS);
			});
	}
...
void NotificationModel::Update (double real_time)
{
	...
	// During intermission, centerprint never expires (matches legacy behavior)
	bool cp_active = !s_state.centerprint.empty () && (g_game_state.intermission || real_time < (s_state.centerprint_expire + CENTERPRINT_FLICKER_SECONDS));
	bool cp_fading = cp_active && !g_game_state.intermission && real_time >= s_state.centerprint_expire;
	...
	for (int i = 0; i < NUM_NOTIFY_LINES; i++)
	{
		...
		bool		active = !line.text.empty () && line.time > 0.0 && elapsed < (notifytime + NOTIFY_FADE_SECONDS);
