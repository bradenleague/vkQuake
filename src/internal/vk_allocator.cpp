...
uint32_t FindVulkanMemoryType (const VkPhysicalDeviceMemoryProperties &mem_props, uint32_t type_filter, VkMemoryPropertyFlags properties)
{
...
std::optional<AllocationResult> FreeListAllocator::Allocate (VkDeviceSize size, VkDeviceSize alignment)
{
	...
	for (size_t i = 0; i < m_free_blocks.size (); ++i)
	{
		...
		else
		{
			...
			block.size = padding;
			m_free_blocks.insert (m_free_blocks.begin () + static_cast<ptrdiff_t> (i) + 1, {remainder_offset, remainder_size});
		}
...
void FreeListAllocator::Free (VkDeviceSize offset, VkDeviceSize size)
{
	...
	auto it =
		std::lower_bound (m_free_blocks.begin (), m_free_blocks.end (), offset, [] (const FreeBlock &block, VkDeviceSize off) { return block.offset < off; });
...
void BufferPool::Initialize (VkDevice device, const VkPhysicalDeviceMemoryProperties &mem_props, VkDeviceSize chunk_size)
{
	m_device = device;
...
bool BufferPool::CreateChunk ()
{
	...
	{
		Rml::Log::Message (Rml::Log::LT_ERROR, "BufferPool: Failed to create chunk buffer (%llu bytes)", static_cast<unsigned long long> (m_chunk_size));
		return false;
	}
	...
	uint32_t mem_type = FindVulkanMemoryType (m_mem_props, mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	...
	{
		Rml::Log::Message (Rml::Log::LT_ERROR, "BufferPool: Failed to allocate chunk memory (%llu bytes)", static_cast<unsigned long long> (mem_reqs.size));
		vkDestroyBuffer (m_device, chunk.buffer, nullptr);
		return false;
	}
	...
	Con_DPrintf ("BufferPool: Created chunk %u (%llu bytes)\n", static_cast<unsigned> (m_chunks.size () - 1), static_cast<unsigned long long> (m_chunk_size));
	return true;
}
...
bool BufferPool::Allocate (VkDeviceSize size, VkDeviceSize alignment, BufferAllocation &out)
{
	...
	{
		Rml::Log::Message (
			Rml::Log::LT_ERROR, "BufferPool: Allocation failed (requested %llu bytes, %u chunks active, %u allocations)",
			static_cast<unsigned long long> (size), static_cast<unsigned> (m_chunks.size ()), m_active_allocations);
		return false;
	}
	...
	{
		Rml::Log::Message (
			Rml::Log::LT_ERROR, "BufferPool: Allocation failed in fresh chunk (requested %llu bytes, chunk size %llu)", static_cast<unsigned long long> (size),
...
void BufferPool::Shutdown ()
{
	...
	{
		Rml::Log::Message (Rml::Log::LT_WARNING, "BufferPool: Shutdown with %u active allocations", m_active_allocations);
	}
...
void ImageMemoryPool::Initialize (
	VkDevice device, const VkPhysicalDeviceMemoryProperties &mem_props, VkDeviceSize buffer_image_granularity, VkDeviceSize page_size)
{
	m_device = device;
	m_mem_props = mem_props;
...
bool ImageMemoryPool::CreatePage (uint32_t memory_type_index)
{
	...
	{
		Rml::Log::Message (
			Rml::Log::LT_ERROR, "ImageMemoryPool: Failed to allocate page (%llu bytes, mem type %u)", static_cast<unsigned long long> (m_page_size),
			memory_type_index);
		return false;
	}
	...
	Con_DPrintf (
		"ImageMemoryPool: Created page %u (%llu bytes, mem type %u)\n", static_cast<unsigned> (m_pages.size () - 1),
		static_cast<unsigned long long> (m_page_size), memory_type_index);
	return true;
}

bool ImageMemoryPool::Allocate (const VkMemoryRequirements &mem_reqs, VkMemoryPropertyFlags properties, ImageMemoryAllocation &out)
{
	uint32_t mem_type = FindVulkanMemoryType (m_mem_props, mem_reqs.memoryTypeBits, properties);
	if (mem_type == UINT32_MAX)
	{
		Rml::Log::Message (
			Rml::Log::LT_ERROR, "ImageMemoryPool: No suitable memory type (filter=0x%x, props=0x%x)", mem_reqs.memoryTypeBits,
	...
	if (mem_reqs.size > m_page_size)
	{
		...
		{
			Rml::Log::Message (
				Rml::Log::LT_ERROR, "ImageMemoryPool: Failed dedicated allocation (%llu bytes)", static_cast<unsigned long long> (mem_reqs.size));
			return false;
		}
	...
	{
		Rml::Log::Message (
			Rml::Log::LT_ERROR, "ImageMemoryPool: Allocation failed (requested %llu bytes, %u pages active, %u allocations)",
			static_cast<unsigned long long> (mem_reqs.size), static_cast<unsigned> (m_pages.size ()), m_active_allocations);
		return false;
	}
	...
	{
		Rml::Log::Message (
			Rml::Log::LT_ERROR, "ImageMemoryPool: Allocation failed in fresh page (requested %llu bytes, page size %llu)",
			static_cast<unsigned long long> (mem_reqs.size), static_cast<unsigned long long> (m_page_size));
		return false;
	}
...
void ImageMemoryPool::Shutdown ()
{
	if (m_active_allocations > 0)
	{
		Rml::Log::Message (Rml::Log::LT_WARNING, "ImageMemoryPool: Shutdown with %u active allocations", m_active_allocations);
	}
